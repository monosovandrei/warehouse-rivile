...
## Методология Telegram-отчета по марже

Эту методологию нужно сохранять одинаковой локально и на production:

- источник: Rivile `GET_I06_LIST`;
- продажи: `I06_OP_TIP=51`, возвраты: `I06_OP_TIP=52`;
- учитываются только проведенные документы `I06_PERKELTA=2`;
...
- количество считается как `1 проведенный заказ = 1 шт.`, не как сумма `I07_KIEKIS`;
- доставка `I07_KODAS=9990006` входит в выручку и маржу менеджера/отдела;
- прочие услуги `I07_TIPAS=2` и остальные коды `999...` не входят в менеджерскую разбивку оборудования;
- менеджер без привязки к отделу остается в общем итоге, но не попадает в отделы.
