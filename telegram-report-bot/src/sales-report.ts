...
function currentMonthReportWindow(now = new Date()) {
...
function emptySalesBucket(): SalesReportLineBucket {
  return { cost: 0, lines: 0, margin: 0, revenue: 0 }
}

function emptySalesGroup(name: string, code?: string, department?: string): MutableSalesReportGroup {
  return { ...emptySalesBucket(), code, department, documentIds: new Set(), documents: 0, name }
}
...
function finalizeSalesGroup(bucket: MutableSalesReportGroup): SalesReportGroup {
  ...
  return {
    ...
    documents: bucket.documentIds.size,
    lines: bucket.lines,
...
export async function loadCurrentMonthSalesReport(): Promise<MonthSalesReport> {
  const window = currentMonthReportWindow()
  ...
  transferredRows.forEach((document) => {
    ...
    lines.forEach((line) => {
      ...
      if (bucket === equipment && departmentName) {
        ...
        addSalesGroupLine(departments.get(departmentName)!, documentId, revenue, signedCost)
        addSalesGroupLine(managerGroups.get(managerCode || managerName)!, documentId, revenue, signedCost)
      }
    })
  })
...
function departmentBreakdownLines(report: MonthSalesReport) {
  ...
  return report.departments.flatMap((department, index) => {
    ...
    const lines = [
      `${department.name}: ${formatReportMoney(department.margin)}`,
      ...managers.map((manager) => `${manager.name}: ${formatReportMoney(manager.margin)}`),
    ]
